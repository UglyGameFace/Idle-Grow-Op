...
source = source[:start] + '''    async def remove_user_cards(self, guild_id: int, user_id: int) -> None:
...
source = source[:start] + '''    async def sync_guild_configuration(self, guild: discord.Guild) -> None:
        config = await self._get_signature_config(guild.id)
...
source = source[:start] + '''    async def disable_guild(self, guild: discord.Guild) -> None:
        config = await self._get_signature_config(guild.id)
        world = await self.bot.db.get_world(guild.id)
        raw_state = world.get(SIGNATURE_STATE_KEY)
        state = dict(raw_state) if isinstance(raw_state, dict) else {}
...
anchor = '''    assert "await duplicate.delete()" in SOURCE
    assert "async def _discover_owned_signatures" not in SOURCE
...
if "test_generation_bump_cancels_only_the_pending_debounce_task" not in core:
