...
def main() -> None:
    ...
    if "class AISetupView" not in source:
...
'''
        anchor = "        view.message = await interaction.original_response()\n\n\nclass Setup(commands.Cog):"
        if anchor not in source:
            raise RuntimeError("Could not locate SetupView insertion point")
        source = source.replace(
            anchor,
            "        view.message = await interaction.original_response()"
            + ai_button
...
'''
    ...
    required = [
        ...
        "async def build_ai_panel",
        'name="🤖 Optional AI"',
    ]
    missing = [item for item in required if item not in source]
    if missing:
        raise RuntimeError(f"AI setup patch incomplete: {missing}")
