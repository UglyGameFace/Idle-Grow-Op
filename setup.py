...
def _permission_health(channel: discord.abc.GuildChannel, member: discord.Member) -> tuple[bool, list[str]]:
...
class ErrorLogChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not isinstance(resolved, (discord.TextChannel, discord.ForumChannel)):
...
class SetupView(discord.ui.View):
    ...
    async def save_error_channel(
        ...
        if guild is None or guild.me is None:
            return await interaction.response.send_message("❌ Server context is unavailable.", ephemeral=True)
        ...
        await interaction.response.edit_message(embed=await self.cog.build_panel(guild), view=self)

    @discord.ui.button(label="Use This Channel", emoji="📍", style=discord.ButtonStyle.primary, row=1)
    async def use_current_channel(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
        channel = interaction.channel
        if not isinstance(channel, (discord.TextChannel, discord.ForumChannel)):
            return await interaction.response.send_message(
                "❌ Open `/setup` in a server text channel to use this option.", ephemeral=True
            )
        await self.save_error_channel(interaction, channel)

    @discord.ui.button(label="Create Log Channel", emoji="➕", style=discord.ButtonStyle.success, row=1)
    async def create_log_channel(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
        guild = interaction.guild
        if guild is None or guild.me is None:
            return await interaction.response.send_message("❌ Server context is unavailable.", ephemeral=True)
        if not guild.me.guild_permissions.manage_channels:
            return await interaction.response.send_message(
                "❌ I need **Manage Channels** to create `idle-grow-logs`.", ephemeral=True
            )
    ...
    @discord.ui.button(label="Send Test", emoji="🧪", style=discord.ButtonStyle.secondary, row=2)
    async def send_test(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
        guild = interaction.guild
        channel = await self.cog.get_error_log_channel(guild) if guild else None
        if channel is None:
            return await interaction.response.send_message(
                "❌ Choose an error logging channel first.", ephemeral=True
            )
        try:
            await channel.send(
                embed=discord.Embed(
    ...
    @discord.ui.button(label="Disable Logging", emoji="🔕", style=discord.ButtonStyle.danger, row=2)
    async def disable_logging(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
        await self.cog.set_error_log_channel(self.guild_id, None)
        await interaction.response.edit_message(
            embed=await self.cog.build_panel(interaction.guild), view=self
        )
...
class Setup(commands.Cog):
    ...
    async def get_error_log_channel(
        ...
        return channel if isinstance(channel, (discord.TextChannel, discord.ForumChannel)) else None

    async def build_panel(self, guild: discord.Guild) -> discord.Embed:
        channel = await self.get_error_log_channel(guild)
        if channel is None:
            ...
            if healthy:
                ...
                status = f"🟠 **Needs attention** — {channel.mention}\nMissing: {', '.join(missing)}"
        ...
        embed.set_footer(text="This private panel expires after 5 minutes. Run /setup anytime to reopen it.")
        return embed
    ...
    async def setup_command(self, ctx: commands.Context) -> None:
        if not isinstance(ctx.author, discord.Member) or not _can_manage_guild(ctx.author):
            return await ctx.send("❌ You need **Manage Server** to configure Idle Grow.", ephemeral=True)

        view = SetupView(self, ctx.author.id, ctx.guild.id)
        message = await ctx.send(embed=await self.build_panel(ctx.guild), view=view, ephemeral=True)
        try:
