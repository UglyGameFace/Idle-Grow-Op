...
def test_crime_uses_only_guild_scoped_storage_paths():
...
def test_crime_sessions_and_rankings_are_guild_isolated():
    source = (ROOT / "crime.py").read_text(encoding="utf-8")

    assert 'return f"guild:{guild_id}:{kind}:{identifier}"' in source
    assert "list_guild_heist_leaderboard(guild_id, limit=10)" in source
