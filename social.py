...
SESH_QUOTES = [
    "Pass it to the left.", "Say less.", "Straight gas no brakes.", 
    "Don't panic, it's organic.", "Certified loud.", "Stay lifted.",
    "Good vibes only.", "Rolling up another one...", "We geekin."
]

# ==========================================================
# 🤝 SUPPORT/VOTE CONFIGURATION
# ==========================================================
SUPPORT_CHANNEL_ID = 1447777409259671732  # Update if needed
SUPPORT_SERVICES = {
...
class Social(commands.Cog):
    ...
    async def profile(self, ctx, target: discord.Member = None):
        ...
        crew_name = "None"
        if user.get("crew_id"):
            c = get_crews().get(str(user["crew_id"]))
            if c: crew_name = c.get("name", "Unknown")
        ...
        embed.add_field(name="💰 Wealth", value=f"Clean: **${user.get('grams', 0):,}**\nDirty: **${user.get('dirty_cash', 0):,}**", inline=False)
        
        stat_text = (f"🌿 Harvested: {stats.get('harvested', 0)}\n"
    ...
    async def crew(self, ctx):
        """Crew commands."""
        await ctx.send("ℹ️ **Crew Commands:**\n`!crew create <name>`\n`!crew join <id>`\n`!crew info`\n`!crew deposit <amount>`\n`!crew war` (Turf War)\n`!district` (Check control)")

    @crew.command(name="create")
    async def crew_create(self, ctx, *, name: str):
        user = self.bot.db.get_user(ctx.author.id)
        if user.get("crew_id"): return await ctx.send("❌ Already in a crew.")
        if user.get("grams", 0) < 50000: return await ctx.send("💸 Cost: $50,000.")
        
        crews = get_crews()
        cid = str(random.randint(10000, 99999))
        crews[cid] = {
            "id": cid, "name": name, "owner_id": ctx.author.id,
            "members": [ctx.author.id], "bank": 0, "level": 1
        }
    ...
    async def crew_join(self, ctx, crew_id: str):
        user = self.bot.db.get_user(ctx.author.id)
        if user.get("crew_id"): return await ctx.send("❌ Leave current crew first.")
        
        crews = get_crews()
        c = crews.get(crew_id)
        if not c: return await ctx.send("❌ Crew not found.")
        
        c["members"].append(ctx.author.id)
        user["crew_id"] = crew_id
        await self.bot.db.save()
        await ctx.send(f"✅ Joined **{c['name']}**!")

    @crew.command(name="deposit")
    async def crew_deposit(self, ctx, amount: int):
        user = self.bot.db.get_user(ctx.author.id)
        cid = user.get("crew_id")
        if not cid: return await ctx.send("❌ No crew.")
    ...
    async def crew_war(self, ctx):
        """Start a turf war for the district."""
        user = self.bot.db.get_user(ctx.author.id)
        cid = user.get("crew_id")
        if not cid: return await ctx.send("❌ You need a crew.")
        ...
        if not current_owner or now >= district.get("expires_at", 0):
            district.update({"owner_crew_id": cid, "owner_name": atk["name"], "multiplier": 1.10, "expires_at": now + 86400})
            await self.bot.db.save()
            return await ctx.send(f"🔥 **{atk['name']}** claimed the empty district!")
            
        if current_owner == cid: return await ctx.send("🏙️ You already own the block.")
        
        # 2. War
        def_crew = crews.get(current_owner)
        if not def_crew: return await ctx.send("❌ Error: Defending crew data missing.")
    ...
    async def district(self, ctx):
        d = self.bot.db.world_state.get("district", {})
        owner = d.get("owner_name", "None")
        mult = int((d.get("multiplier", 1.0) - 1) * 100)
        rem = max(0, int((d.get("expires_at", 0) - time.time()) / 60))
    ...
    async def sesh(self, ctx, *, args: str = ""):
        """Start a sesh (Creates Voice Channel)."""
    ...
    async def _start_sesh_flow(self, ctx, args, kind):
        ...
        _ACTIVE_SESHES[msg.id] = {
            "guild_id": guild.id, "host_id": host.id, "vc_id": vc.id if vc else None,
            "private": created_private, "expiry": time.time() + 600
        }
        
        # 6. Move users if private
        if created_private and vc:
            try:
                await host.move_to(vc)
                if target: await target.move_to(vc)
            except: pass
    ...
    async def _create_private_vc(self, guild, host, target):
        ...
        cat = host.voice.channel.category if host.voice else None
        name = f"private-sesh-{host.display_name[:5]}"
        return await guild.create_voice_channel(name=name, category=cat, overwrites=overwrites)
    ...
    # --- LISTENER: VOTE REWARDS ---
    @commands.Cog.listener()
    async def on_message(self, message):
        if message.channel.id != SUPPORT_CHANNEL_ID: return
        if message.author.bot or message.author.id in SUPPORT_SERVICES:
            content = message.content.lower() + " ".join([e.description.lower() for e in message.embeds if e.description])
            ...
            if "bump done" in content or "voted" in content:
                ...
                u_data = self.bot.db.get_user(user.id)
                u_data["xp"] = int(u_data.get("xp", 0)) + SUPPORT_REWARD_XP
                await self.bot.db.save()
                await message.channel.send(f"✅ **{user.mention}** received {SUPPORT_REWARD_XP} XP for voting!")
...
class SeshView(discord.ui.View):
    ...
    async def join_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        sess = _ACTIVE_SESHES.get(interaction.message.id)
        if not sess or not sess.get("vc_id"):
            return await interaction.response.send_message("❌ No VC attached.", ephemeral=True)
            
        vc = interaction.guild.get_channel(sess["vc_id"])
        if vc:
    ...
    async def end_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.host_id: return
        sess = _ACTIVE_SESHES.get(interaction.message.id)
        if sess and sess.get("vc_id"):
            vc = interaction.guild.get_channel(sess["vc_id"])
